...
bool detector(vpImage<vpRGBa> &I, double tag_size, const vpCameraParameters &cam,
              vpHomogeneousMatrix &c_M_o, std::vector<vpImagePoint> &corners, PelotaDetector &detectorPelota)
{
  bool ret;

  ret = detectorPelota.detectPelota(I, corners);
  if (ret) {
...
int main(int argc, char **argv)
{
  ...
  //Inicializamos detector de pelota con YOLO
  std::cout << "Cargando modelo YOLO..." << std::endl;
  PelotaDetector detector_pelota("src/modelo_yolo/yolov4-custom/yolov4-tiny-custom.cfg", "src/modelo_yolo/yolov4-custom/yolov4-tiny-custom_last.weights");
  ...
  try {
    ...
    while (!has_converged && !final_quit) {
      ...
      bool ret = detector(I, pelota_size, cam, c_M_o, corners, detector_pelota);
      ...
      double max_vel_trans = 0.1; // Límite traslacional: 10 cm/s
