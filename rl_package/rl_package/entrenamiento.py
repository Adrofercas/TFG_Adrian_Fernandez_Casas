...
class RosEnv(gym.Env):
    def __init__(self, a, b, c, d, max_steps=500):
        ...
        # === Espacios ===
        obs_high = np.array([2.9007, 1.8361, 2.9007, -0.1169, 2.8763, 4.6216, 3.0508, 2.9007, 1.8361, 2.9007, -0.1169, 2.8763, 4.6216, 3.0508, 10, 10, 10, 10, 10, 10, 10], dtype=np.float32)
        obs_low = np.array([-2.9007, -1.8361, -2.9007, -3.0770, -2.8763, 0.4398, -3.0508, -2.9007, -1.8361, -2.9007, -3.0770, -2.8763, 0.4398, -3.0508, -10, -10, -10, -10, -10, -10, -10], dtype=np.float32)
        self.observation_space = spaces.Box(obs_low, obs_high, dtype=np.float32)

        # Acción en [0, 1] (más estable para algoritmos)
        self.act_high = np.array([2000., 2000., 2000., 2000., 2000., 3000., 1500.,
                             1000., 1000., 1000., 1000., 1000., 1000., 1000.], dtype=np.float32)
        self.action_space = spaces.Box(low=-np.ones(14, dtype=np.float32),
        ...
        self.angle_threshold = 10.0  # umbral angular para considerar alcanzada la meta
    ...
    def step(self, action):
        ...
        media_rew = 0.0
        if self.total_eps > 250: #cuando llene el buffer de episodios, miro si ha conseguido aprender lo suficiente
        ...
        if self.current_step % 200 == 0:
            ...
            self.k_action = (action[:7] + 1) * (self.act_high[:7] / 2)
            self.d_action = (action[7:] + 1) * (self.act_high[7:] / 2)
        ...
        if np.mean(np.abs(self.joint_jerks)) > 10:
            self.total_reward -= 1.0  # penalización por jerk alto
        ...
        if dist2goal < self.dist_threshold and rot2goal < self.angle_threshold:
            ...
            terminated = True
            self.rew_eps = np.append(self.rew_eps, 1) # añado recompensa episodio si termina correctamente
        else:
            self.rew_eps = np.append(self.rew_eps, 0) # añado 0 si no termina bien
        ...
        self.mean_dist += dist2goal
        self.mean_rot += rot2goal
        ...
        if truncated or terminated:
            ...
            self.logger.record(f"parameters/dist2goal", float(self.mean_dist/self.current_step))
            self.logger.record(f"parameters/rot2goal", float(self.mean_rot/self.current_step))
...
def main():
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=5_000,
        save_path="/home/afernandez/colcon_ws/src/rl_package/saves/all-joints_modified",
        name_prefix="all-joints"
    )

    # entropyCallback = AdaptiveEntropyCallback(check_freq=8000, decay=0.97, min_ent=0.08)

    env = RosEnv(a=18, b=25, c=2, d=2, max_steps=10_000)
    env = Monitor(env)
    ...
    entropy_parameters = [0.001, 0.3, 0.5, 0.1] # max_change, min_value, max_value, loss_dampening

    model = SAC("MlpPolicy", env, verbose=1,
                learning_rate=3e-4, buffer_size=200_000, learning_starts=10_000, batch_size=256, tau=0.01, gamma=0.98, ent_coef='auto', train_freq=(200, 'step'),
    ...
    #vamos a seguir con el training
    # model = SAC.load(
    #     "/home/afernandez/colcon_ws/src/rl_package/saves/all-joints_modified/all-joints_175000_steps.zip",
    #     env=env,
    ...
    #     reset_num_timesteps=True,    # continúa desde donde estaba
    #     log_interval=1,
    #     callback=callback
    # )
