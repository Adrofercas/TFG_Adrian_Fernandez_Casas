...
class FrankaJointImpedanceController:
    """
    Controlador de impedancia con Generador de Trayectoria (Motion Profiler).
    Evita saltos bruscos de aceleración (Jerk infinito) interpolando el objetivo.
    """
    ...
    def __init__(self, robot, robot_view):
        ...
        # --- NUEVO: Variables de Trayectoria ---
        # Límites físicos seguros para el Franka (rad/s y rad/s^2)
        # Puedes ajustarlos: más bajos = movimiento más suave, más altos = más agresivo
        self.max_vel = np.array([2.0, 2.0, 2.0, 2.0, 2.5, 2.5, 2.5]) * 0.5  # Factor 0.5 por seguridad
        self.max_acc = np.array([2.5, 2.5, 2.5, 2.5, 3.5, 3.5, 3.5]) * 0.2  # Factor 0.2 para reducir Jerk inicial
        
        # Estado del generador de trayectoria
        self.current_ref_pos = None  # El target "suave" actual
        self.current_ref_vel = np.zeros(7) # Velocidad de referencia actual
        self.final_goal_pos = None   # El objetivo final real (input del usuario)
    ...
    def set_target(self, joint_positions):
        """
        Establece la posición INICIAL o fuerza un salto (usar solo al inicio)
        """
        target = np.array(joint_positions, dtype=float)
        # Si es la primera vez, inicializamos todo igual para evitar salto
        if self.current_ref_pos is None:
            self.current_ref_pos = target.copy()
            self.final_goal_pos = target.copy()
        else:
            # Si ya está corriendo, esto cuenta como un nuevo objetivo final
            self.final_goal_pos = target
    ...
    def update_target(self, new_target):
        """
        Actualiza SOLO el objetivo final. El interpolador se encargará de llegar ahí suavemente.
        """
        self.pending_target_update = np.array(new_target, dtype=float)

    def apply_pending_updates(self):
        if self.pending_k_update is not None and len(self.pending_k_update) == 7:
            self.stiffness = self.pending_k_update
            self.pending_k_update = None

        if self.pending_d_update is not None and len(self.pending_d_update) == 7:
            self.damping = self.pending_d_update
            self.pending_d_update = None

        if self.pending_target_update is not None and len(self.pending_target_update) == 7:
            # AQUI ESTÁ LA CLAVE: No sobreescribimos current_ref_pos, solo el destino final
            self.final_goal_pos = self.pending_target_update
            self.pending_target_update = None

    def _step_trajectory(self, dt=0.001):
        """
        Calcula el siguiente punto de la trayectoria basado en aceleración constante.
        Genera el perfil: Empieza lento -> Acelera -> Mantiene Vel -> Desacelera.
        """
        if self.current_ref_pos is None or self.final_goal_pos is None:
            return

        for i in range(7):
            # 1. Distancia al objetivo
            dist = self.final_goal_pos[i] - self.current_ref_pos[i]
            
            # Si ya llegamos (con margen pequeño), paramos velocidad
            if abs(dist) < 1e-4:
                self.current_ref_vel[i] = 0.0
                self.current_ref_pos[i] = self.final_goal_pos[i]
                continue

            # 2. Dirección de movimiento
            direction = np.sign(dist)
            
            # 3. Velocidad máxima permitida por distancia (Física: v^2 = 2*a*d) para poder frenar a tiempo
            # Esto asegura que desacelere al acercarse al final
            max_vel_braking = np.sqrt(2 * self.max_acc[i] * abs(dist))
            
            # La velocidad deseada es la menor entre: límite global y límite de frenado
            target_vel = direction * min(self.max_vel[i], max_vel_braking)
            
            # 4. Calcular cambio de velocidad (Aceleración)
            # Limitamos el cambio de velocidad a max_acc * dt
            vel_diff = target_vel - self.current_ref_vel[i]
            max_vel_change = self.max_acc[i] * dt
            
            # Clamp del cambio de velocidad
            vel_step = np.clip(vel_diff, -max_vel_change, max_vel_change)
            
            # 5. Integrar
            self.current_ref_vel[i] += vel_step
            self.current_ref_pos[i] += self.current_ref_vel[i] * dt

    def compute_impedance_torques(self):
        if self.current_ref_pos is None:
            return np.zeros(7)

        # A. Actualizar la trayectoria suave (INTERPOLACIÓN)
        # Asumimos dt = 0.001 (1ms) que es tu physics step, o calculamos real si quieres
        self._step_trajectory(dt=0.001)
        ...
        try:
            ...
            # C. Calcular Error usando la REFERENCIA SUAVE (current_ref_pos), NO el final_goal
            # Esto hace que el error sea pequeño al principio y crezca solo lo que permite la aceleración
            position_error = self.current_ref_pos - q
            
            # Opcional: Feed-forward de velocidad si quisieras tracking perfecto
            # velocity_error = self.current_ref_vel - dq
            
            # Ley de control
            # Usamos self.stiffness * position_error - self.damping * dq
...
class FrankaImpedanceSimulation:
    ...
    def on_update(self, event):
        ...
        try:
            ...
            q_error = self.controller.final_goal_pos - q
        except Exception as e:
